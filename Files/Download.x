...
// ============================================================
// MARK: - Helpers & Utilities (Pure Functions)
// ============================================================
...
static NSString *YouModURLStringBypassingThrottle(NSString *urlString) {
    ...
    if (components) {
        ...
        for (NSURLQueryItem *item in queryItems) {
            if (![item.name isEqualToString:@"n"]) [filtered addObject:item];
        }
        BOOL hasRateBypass = NO;
        for (NSURLQueryItem *item in filtered) {
            if ([item.name isEqualToString:@"ratebypass"]) { hasRateBypass = YES; break; }
        }
        if (!hasRateBypass) [filtered addObject:[NSURLQueryItem queryItemWithName:@"ratebypass" value:@"yes"]];
...
// متغير عام للتوكن (يُملأ عن طريق hooks)
...
static CMTime YouModExportDuration(AVAsset *videoAsset, AVAsset *audioAsset, unsigned long long expectedDurationMs) {
    ...
    if (YouModCMTimeIsUsable(mediaDuration) && CMTIME_COMPARE_INLINE(duration, >, mediaDuration)) return mediaDuration;
    return duration;
}
...
// ============================================================
// MARK: - YouTube Data Extraction Service
// ============================================================
...
static NSArray *YouModCaptionTracksForPlayer(YTPlayerViewController *player) {
    ...
    return tracklistRenderer.captionTracksArray;
}
...
// ============================================================
// MARK: - UI Helpers (Notifications, Menus, ViewControllers)
// ============================================================
...
static void YouModShowDownloadCompleteDialog(NSString *message, void (^saveHandler)(void), void (^shareHandler)(void)) {
    UIView *parent = sbGetNotificationParent();
...
static void YouModSaveImageToPhotos(UIImage *image, UIViewController *presenter, void (^completion)(BOOL success, NSError *error)) {
    YouModRequestPhotoAccess(^(BOOL granted) {
        if (!granted) {
            NSError *error = [NSError errorWithDomain:@"YouMod" code:1 userInfo:@{NSLocalizedDescriptionKey: @"Photos access denied"}];
            completion(NO, error);
...
static void YouModHandlePostDownloadImage(UIImage *image, UIViewController *presenter) {
    ...
    if (action == PostDownloadActionSaveToPhotos) {
        ...
        YouModShowDownloadCompleteDialog(LOC(@"DOWNLOAD_COMPLETED"), ^{
            YouModSaveImageToPhotos(image, presenter, ^(BOOL success, NSError *error) {
                if (success) YouModSendSuccess(LOC(@"SAVED_TO_PHOTOS"));
                else YouModSendError(error.localizedDescription ?: LOC(@"SAVE_FAILED"));
            });
        }, ^{
            YouModShareItem(image, presenter);
        });
...
// --- Thumbnail Viewer & Translation View Controllers (placeholders) ---
// (سيتم وضع الكود الفعلي لهذه الفئات هنا، لكن اختصاراً نتركها كما هي في الملف الأصلي)

// ============================================================
// MARK: - Range Downloader (Networking)
// ============================================================
...
- (void)cancel {
    dispatch_async(self.stateQueue, ^{
        ...
        dispatch_async(self.fileQueue, ^{
            @try { [self.fileHandle closeFile]; } @catch (__unused NSException *exception) {}
            [NSFileManager.defaultManager removeItemAtURL:self.destinationURL error:nil];
        });
    });
}
...
- (NSError *)validationErrorForChunk:(YouModDownloadChunk *)chunk data:(NSData *)data response:(NSURLResponse *)response error:(NSError *)error {
    ...
    if (httpResponse && !statusOK) return [self errorWithCode:24 message:@"Range request rejected by server"];
    if (data.length != chunk.length) return [self errorWithCode:25 message:@"Incomplete chunk"];
    return nil;
}
...
// ============================================================
// MARK: - Download Coordinator (Main Orchestrator)
// ============================================================
...
- (void)showProgressWithTitle:(NSString *)title presenter:(UIViewController *)presenter {
    ...
    __weak typeof(self) weakSelf = self;
    self.progressPill = [YMDownloadProgressView showInView:sbGetNotificationParent()
        message:[NSString stringWithFormat:@"%@ - 0%%", title]
        cancelAction:^{ [weakSelf cancelWithMessage:LOC(@"DOWNLOAD_CANCELLED")]; }];
}
...
// --- Direct Download Methods (Video & Audio) ---
- (void)startVideoDownloadWithVideoFormat:(YouModMediaFormat *)videoFormat audioFormat:(YouModMediaFormat *)audioFormat fileName:(NSString *)fileName presenter:(UIViewController *)presenter videoID:(NSString *)vidID {
    if (self.active) { YouModSendToast(LOC(@"ALREADY_DOWNLOADING")); return; }
...
- (NSString *)serverEndpoint {
    if (INTFORVAL(DownloadServerIndex) == 0) {
        return @"https://appropriatenet2928.tail6a9ca7.ts.net/";
    } else if (INTFORVAL(DownloadServerIndex) == 1) {
        return @"https://waterserver.freeddns.org/";
    }
    return @"";
}
...
- (void)pollJobStatus:(NSString *)jobId isAudio:(BOOL)isAudio presenter:(UIViewController *)presenter completion:(void (^)(NSURL *localURL, NSString *errorMsg))completionBlock {
    ...
    [[[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if ([status isEqualToString:@"done"]) {
            NSString *singleFileName = json[@"filename"] ?: (isAudio ? @"downloaded_file.mp3" : @"downloaded_file.mp4");
            ...
            dispatch_async(dispatch_get_main_queue(), ^{ [self updateProgressTitle:LOC(@"DOWNLOADING_TO_SERVER") progress:0.0f]; });
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self pollJobStatus:jobId isAudio:isAudio presenter:presenter completion:completionBlock];
            });
        }
...
- (void)downloadSingleFile:(NSString *)filename isAudio:(BOOL)isAudio forJobId:(NSString *)jobId presenter:(UIViewController *)presenter completion:(void (^)(NSURL *localURL, NSString *errorMsg))completionBlock {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{ [self updateProgressTitle:self.baseProgressTitle progress:0.0f]; });
...
// ============================================================
// MARK: - Action Handlers (UI Interaction)
// ============================================================
...
static void YouModShowThumbnailViewer(YTPlayerViewController *player, UIViewController *presenter) {
    ...
    if (!thumbnailURL) { YouModSendError(LOC(@"NO_THUMBNAIL_FOUND")); return; }
...
static void YouModShowCaptionsSheet(YTPlayerViewController *player, UIViewController *presenter, UIView *sender) {
    ...
    if (tracks.count == 0) { YouModSendError(LOC(@"NO_CAPTIONS")); return; }
    NSMutableArray *items = [NSMutableArray array];
    for (YTICaptionTrackEntry *track in tracks) {
        ...
        [items addObject:[YouModMenuItem itemWithTitle:nameStr subtitle:languageCode icon:YouModYTIconImage(50, NO, nil) handler:^{
            ...
            if (!url) { YouModSendError(LOC(@"NO_CAPTIONS_URL")); return; }
            YouModSendToast(LOC(@"DOWNLOADING_CAPTIONS"));
            [[NSURLSession.sharedSession dataTaskWithURL:url completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                if (error || data.length == 0) {
                    dispatch_async(dispatch_get_main_queue(), ^{ YouModSendError(LOC(@"CAPTIONS_FAILED")); });
                    return;
                }
                NSString *title = YouModTitleForPlayer(player);
    ...
    if (items.count == 0) { YouModSendError(LOC(@"NO_CAPTIONS_URL")); return; }
    YouModPresentMenu(nil, items, presenter, sender);
}

// --- Download Quality/Audio Selection ---
...
static void YouModShowVideoQualitySheet(YTPlayerViewController *player, UIViewController *presenter, UIView *sender, BOOL isShorts) {
    ...
    NSString *title = YouModTitleForPlayer(player);
    if (videoFormats.count == 0) { YouModSendError(LOC(@"NO_VID_AUDIO_STREAM_FOUND")); return; }
    NSMutableArray *items = [NSMutableArray array];
    for (YouModMediaFormat *format in videoFormats) {
        NSString *rowTitle = format.qualityLabel;
        NSString *subtitle = YouModFormatSubtitle(format, YES);
        [items addObject:[YouModMenuItem itemWithTitle:rowTitle subtitle:subtitle icon:YouModYTIconImage(isShorts ? 769 : 658, NO, nil) handler:^{
...
static void YouModShowDownloadManager(YTPlayerViewController *player, UIViewController *presenter, UIView *sender, BOOL isShorts) {
    if (!player) { YouModSendError(LOC(@"OPEN_VID_BEFORE")); return; }
    NSMutableArray *items = [NSMutableArray array];
    YTSingleVideoController *sgvidcon = player.activeVideo;
    YTSingleVideo *sgvid = sgvidcon.singleVideo;
    if (!sgvid.isLivePlayback) {
        [items addObject:[YouModMenuItem itemWithTitle:isShorts ? LOC(@"DOWNLOAD_SHORTS") : LOC(@"DOWNLOAD_VIDEO") subtitle:nil icon:YouModYTIconImage(isShorts ? 769 : 658, NO, nil) handler:^{
            YouModShowVideoQualitySheet(player, presenter, sender, isShorts);
        }]];
...
// --- Comment & Post Long Press Handlers ---
...
- (void)viewWillDisappear:(BOOL)animated {
    %orig;
    if (YouModCurrentPlayerViewController == self) YouModCurrentPlayerViewController = nil;
}
%end
...
// --- Configure Download Button in _ASDisplayView ---
...
- (void)didMoveToWindow {
    ...
    if ([self.accessibilityIdentifier isEqualToString:@"id.ui.comment_cell"] && IS_ENABLED(DownloadComment)) {
        ...
        for (UIGestureRecognizer *g in self.gestureRecognizers) {
            if ([g.name isEqualToString:@"YouModCommentLongPress"]) { hasGesture = YES; break; }
        }
        if (!hasGesture) {
    ...
    if ([self.accessibilityIdentifier isEqualToString:@"id.ui.backstage.original_post"] && IS_ENABLED(DownloadPost)) {
        BOOL hasGesture = NO;
        for (UIGestureRecognizer *g in self.gestureRecognizers) {
            if ([g.name isEqualToString:@"YouModPostLongPress"]) { hasGesture = YES; break; }
        }
        if (!hasGesture) {
...
- (void)YouModHandleCommentLongPress:(UILongPressGestureRecognizer *)sender {
    ...
    [items addObject:[YouModMenuItem itemWithTitle:LOC(@"COPY_COMMENT_IMAGE") subtitle:nil icon:YouModYTIconImage(208, NO, nil) handler:^{
        UIImage *image = YouModRenderViewToImage(self);
        if (image) YouModCopyImageToPasteboard(image, @"COPIED_TO_CLIPBOARD");
    }]];
    UIViewController *presenter = YouModPresenterForSender(self, nil);
    if (!presenter) return;
